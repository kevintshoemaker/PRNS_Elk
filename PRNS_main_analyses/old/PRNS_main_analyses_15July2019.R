...
alldata$rut <- readRDS("rut.obs.15Jul2019.rds")              
alldata$parturition <- readRDS("part.obs.15Jul2019.rds")
alldata$summer <- readRDS("summer.obs.15Jul2019.rds")
alldata$winter <- readRDS("winter.obs.15Jul2019.rds")
...
allseasons2 <- allseasons
allseasons2[1] <- "breeding"


graphics.off()

svg("coefplot_final.svg",7.5,5.5)

layout(matrix(1:12,nrow=3,byrow = T),widths = c(0.7,2,2,2))          
var_ndx <- 1
onleft <- c(1,5,9)
for(panel in c(1:12)){
  ...
  if(panel %in% onleft){
    ...
    plot(20,20,xlim=c(8,25),ylim=c(-2.5,16),xlab="",ylab="",xaxt="n",yaxt="n",main=varlist2$rut$readable[var_ndx],pch="")    # blank plot
    xcents <- c(11,17,23)
...
for(panel in c(1:6)){
  ...
  if(panel %in% onleft){
    ...
    plot(20,20,xlim=c(9,32),ylim=c(-2.5,16),xlab="",ylab="",xaxt="n",yaxt="n",main=varlist2$rut$readable[var_ndx],pch="")    # blank plot
    xcents <- c(14,22,30)
    #abline(v=xcents,lwd=2)
    ycents <- seq(15,3,length=4)
    yshift <- 0
