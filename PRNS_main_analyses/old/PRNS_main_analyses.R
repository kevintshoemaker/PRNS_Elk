...
VisualizeRelation <- function(data=df,model=full,predvar="std.slope",varname="Slope",allvars=names(df)[-c(1,6:13)],season,svg=F){
  ...
  if(svg){
    ...
    plot(range,1-pred_pz$fit,xlab=varname,ylab="P(1+)",type="l",lwd=2,xaxt="n",ylim=c(0,1))
...
###############
# Reviewer comment about independence

temp <- final.data$summer
subset(temp,ID=="aaaa")
alldates <- 
...
while(panel < 25){
  if(panel %in% onleft){
    ...
    if(panel==22){
      plot(1,1,xlim=c(0,1),ylim=c(0,1),bty="n",xlab="",ylab="",xaxt="n",yaxt="n",main="",pch="")
      text(0.5,0.5,"Relative Selection Intensity",srt=90,adj=0.5,cex=1.5)
      panel=panel+1
    }else{
      plot(1,1,xlim=c(0,1),ylim=c(0,1),bty="n",xlab="",ylab="",xaxt="n",yaxt="n",main="",pch="")
