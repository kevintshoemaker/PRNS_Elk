#GLM's for PRNS elk project (Collar data). Created by Lacey Hughey May 7, 2019
...
build_indmodels <- function(thisind=indnames[2]){
  ...
  full <- tryCatch( {     
    glm(Used ~ std.pasture + std.stocking + std.ponds + std.elevation +
          std.insolation +std.ndvi + std.scrub + std.slope +
          std.ndvi:std.scrub + std.stocking:std.ndvi,
        data= dat, family=binomial(link= "logit"))
  }
  , warning = function(w) { 
    
    f <- logistf::logistf(Used ~ std.pasture + std.stocking + std.ponds + std.elevation +
                            std.insolation +std.ndvi + std.scrub + std.slope +
                            std.ndvi:std.scrub + std.stocking:std.ndvi,
                          data= dat, family=binomial(link= "logit"))
    return(f)
...
allvar_c <- c('Used','std.pasture','std.stocking','std.ponds','std.elevation',
                'std.insolation','std.ndvi','std.scrub','std.slope')

VisualizeRelation_col <- function(data=allcollar[[season]],meancoefs=thisres,predvar="std.insolation",allvars=allvar_c,season){
  len <- 100
  ...
  tcrit <- qt(0.95,nrow(meancoefs))
  
  par(mai=c(0.9,0.8,0.1,0.1))
  plot(range,pred,xlab=predvar2,ylab="Selection propensity",type="l",lwd=2,xaxt="n",ylim=c(0,0.25))
...
VisualizeInteraction_col <- function(data=df,meancoefs=thisres_f,var1="std.stocking",var2="std.ndvi",allvars=allvar_c,season){
...
#load data
allcollar$rut <- readRDS("all.extracts.rut.2May2019.rds")
allcollar$part <- readRDS("all.extracts.part.2May2019.rds")
allcollar$summer <- readRDS("all.extracts.summer.2May2019.rds")
allcollar$winter <- readRDS("all.extracts.winter.2May2019.rds")

##############
# Process data

#replace with subset of columns of interest for appropriate season 
allcollar2 <- lapply(allcollar,function(t) t[c(3:5, 9:12, 13:14, 17:26)])
...
standardze2 <- function(df){
  ...
  df$std.streams <- ((df$streams)-(mean(df$streams)))/(2*sd(df$streams))
  return(df)
}
...
for(season in allseasons){
  ...
  allinds <- allinds[indnames]
      ...
      # scale the number of background points to the number of observations
  ...
  avails2 <- lapply(indnames, function(t) 
    avails[[season]][sample(1:nrow(avails[[season]]),size=min(nrow(avails[[season]]),nobs[t]*5),replace=FALSE),])
  names(avails2) <- indnames
  
  allinds <- lapply(indnames,function(t) rbind(allinds[[t]], avails2[[t]]) )
  names(allinds) <- indnames
  

  
  allmods <- lapply(indnames,build_indmodels)
  names(allmods) <- indnames
  
  summary(allmods$`24034B`)
  
  
  
  sapply(allinds,nrow)
  
  coefs <- sapply(allmods,coefficients)
  ...
  svg(sprintf("collar_coefs_all_%s.svg",season),width = 4,height=5)
    ...
    p<- ggplot(thisres,aes(x=meancoefs, y=names)) + 
      ...
      geom_errorbarh(aes(xmin=meancoefs-secoefs*tcrit, xmax=meancoefs+secoefs*tcrit),height=0.2)
    print(p)
  ...
  tmp <- as.character(thisres_f$names[grepl("std",thisres_f$names)])
  allmains <- tmp[!grepl(":",tmp)]
  allints <- tmp[grepl(":",tmp)]
  tmp <- sapply(1:length(allmains), function(t) VisualizeRelation_col(data=df,meancoefs=thisres_f,predvar = allmains[t],allvars=allmains,season=season) )     # run and save partial dependence plots for all top variables
  
  ints <- strsplit(allints[grep(":",allints)],":")
  tmp <- sapply(1:length(ints), function(t) VisualizeInteraction_col(data=df,meancoefs=thisres_f,var1=ints[[t]][1],var2=ints[[t]][2],allvars=allmains,season=season) )
}
